...
struct Vec{
    ...
    Vec operator+(const Vec& rhs) const { return {x + rhs.x, y + rhs.y, z + rhs.z}; }
    Vec operator-(const Vec& rhs) const { return {x - rhs.x, y - rhs.y, z - rhs.z}; }
    Vec operator*(const Vec& rhs) const { return {x * rhs.x, y * rhs.y, z * rhs.z}; }
    Vec operator/(const Vec& rhs) const { return {x / rhs.x, y / rhs.y, z / rhs.z}; }
    Vec operator*(double scalar)     const { return {x * scalar, y * scalar, z * scalar}; }
    ...
    // Utility methods
    Vec opposite() const { return {-x, -y, -z}; }
    ...
    Vec normalize() const {
        double len = length();
        return (len != 0) ? *this * (1.0 / len) : Vec(0, 0, 0);
    }

    Vec clamp() const{
        Vec output = Vec(x, y, z);
        output = Vec(
            x > 1 ? 1 : x,
            y > 1 ? 1 : y,
            z > 1 ? 1 : z);
...
struct Ray
{
    ...
    Ray(const Vec &origin, const Vec &direction, int depth) : origin(origin), direction(direction), depth(depth) {}
